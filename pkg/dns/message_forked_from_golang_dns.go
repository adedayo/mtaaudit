...
//Forked from Go dnsmessage, which is Copyright 2009 The Go Authors. All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

//The fork is necessary to support packing and unpacking of DNS types that are not covered under the standard
//Go implementation

// Package dns provides a mostly RFC 1035 compliant implementation of
...
//Wire format of supported types
const (
	...
	TypeSRV   Type = 33
	TypeOPT   Type = 41
	TypeTLSA  Type = 52 //TLSA type for DANE
...
//Wire format of network classes
const (
...
//Wire format of DNS response status codes
const (
	// Message.Rcode
...
//GoString string format of resource
...
func (m *Message) GoString() string {
	s := "dns.Message{Header: " + m.Header.GoString() + ", " +
...
func (n *Name) GoString() string {
	return `dns.MustNewName("` + printString(n.Data[:n.Length]) + `")`
}
...
func unpackResourceBody(msg []byte, off int, hdr ResourceHeader) (ResourceBody, int, error) {
	...
	case TypeOPT:
		...
		r = &rb
		name = "OPT"
	case TypeTLSA: //process TLSA resource
		var rb TLSAResourceData
		rb, err = unpackTLSAResourceData(msg, off, hdr.Length)
		rb.Name = hdr.Name.String()
		r = &rb
		name = "TLSA"
	}
	if err != nil {
...
func (r *CNAMEResource) GoString() string {
	return "dns.CNAMEResource{CNAME: " + r.CNAME.GoString() + "}"
}
...
func (r *NSResource) GoString() string {
	return "dns.NSResource{NS: " + r.NS.GoString() + "}"
}
...
func (r *PTRResource) GoString() string {
	return "dns.PTRResource{PTR: " + r.PTR.GoString() + "}"
}
...
func (r *OPTResource) GoString() string {
	s := "dns.OPTResource{Options: []dns.Option{"
	if len(r.Options) == 0 {
		return s + "}}"
	}
